...
FilterOnePole lowPassFilter(LOWPASS, 0.003183);  // FILTER_LP = Low-pass filter
FilterOnePole highPassFilter(HIGHPASS, 0.3183); // HPF = High-Pass Filter, 0.1 = time constant
 ...
 */
  ads.setGain(ADS1220_GAIN_128); // set gain to 8
  ads.bypassPGA(false);  // redundant since this is default.
...
void loop(){
  ...
  result_mv = ads.getVoltage_mV();
  longResult = ads.getRawData();
  Serial.print("AIN0 vs. AIN1  [mV]: ");
  Serial.println(result_mv);

  float result_uv = result_mv * 1000;

  Serial.print("AIN0 vs. AIN1  [uV]: ");
  Serial.println(result_uv);

  // Low-pass filter
  float filteredLowPass = lowPassFilter.input(result_uv);
  Serial.print("Low-pass filtered signal: ");
  Serial.println(filteredLowPass);
  
  // High-pass filter
  float filteredHighPass = highPassFilter.input(result_uv);
  Serial.print("High-pass filtered signal: ");
  Serial.println(filteredHighPass);

  // Band-pass filter
  float filteredBandPass = highPassFilter.input(filteredLowPass);
  Serial.print("Band-pass filtered signal: ");
  Serial.println(filteredBandPass);
